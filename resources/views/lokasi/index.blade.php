    ...
    <div class="container-fluid px-3 px-md-4 py-4" style="background:#f8fafc;min-height:100vh">
        <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3 mb-4">
            <div>
                ...
                <h1 class="h2 fw-bold text-dark mb-1">Kelola Lokasi</h1>
                <p class="text-secondary mb-0">Atur lokasi penyimpanan agar pencarian aset lebih cepat.</p>
            </div><a href="{{ route('lokasi.create') }}" class="btn btn-success btn-lg rounded-3 fw-bold"><i
                    class="fas fa-plus-circle me-2"></i>Tambah Lokasi Baru</a>
        </div>
        @if (session('success'))
            ...
            @endif @if (session('error'))
                ...
                <div class="card border-0 rounded-4 shadow-sm overflow-hidden">
                    <div class="bg-light p-3 border-bottom">
                        <form method="GET" action="{{ route('lokasi.index') }}" class="row g-2 align-items-end">
                            <div class="col-md-6"><label for="search" class="form-label small fw-bold text-secondary">Cari
                                    lokasi</label><input type="text" id="search" name="search"
                                    value="{{ request('search') }}" class="form-control rounded-3"
                                    placeholder="Ketik nama lokasi..."></div>
                            ...
                            <div class="col-auto"><a href="{{ route('lokasi.index') }}"
                            ...
                            <tbody>
                                @forelse($lokasis as $index => $lokasi)
                                    <tr class="border-bottom">
                                        <td class="px-4">{{ $lokasis->firstItem() + $index }}</td>
                                        <td><span
                                                class="badge rounded-pill bg-info-subtle text-info-emphasis border border-info-subtle px-3 py-2">{{ $lokasi->nama_lokasi }}</span>
                                        </td>
                                        <td>{{ $lokasi->deskripsi ?? '-' }}</td>
                                        <td class="text-secondary">
                                            {{ optional($lokasi->created_at)->format('d-m-Y H:i:s') }}</td>
                                        <td class="text-secondary">
                                            {{ optional($lokasi->updated_at)->format('d-m-Y H:i:s') }}</td>
                                        <td class="px-4">
                                        ...
                                        <td colspan="6" class="py-5 text-center"><i
                                                class="fas fa-map-marker-alt fa-2x text-secondary mb-3"></i>
                                            <h5 class="fw-bold">Belum ada lokasi</h5>
                                            <p class="text-secondary">Tambahkan lokasi penyimpanan pertama.</p><a
                    ...
                    <div class="p-3">{{ $lokasis->links() }}</div>
                </div>
    </div>
@endsection
