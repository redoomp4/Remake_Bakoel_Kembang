...
@section('content')
    <div class="min-h-full bg-brand-offwhite">
        <main class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
            <header
                class="mb-8 flex flex-col gap-4 border-b border-brand-accent/60 pb-6 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    ...
                    <p class="mt-2 max-w-2xl text-sm leading-6 text-brand-slate">Input master data, item baru, dan transaksi
                        tanpa berpindah halaman.</p>
                </div>
                <a href="{{ url()->previous() }}"
                    class="inline-flex w-fit items-center gap-2 rounded-xl border border-brand-accent bg-white px-4 py-3 text-sm font-bold text-brand-slate shadow-sm transition hover:border-brand-emerald hover:text-brand-emerald">
            ...
            <div class="mb-6 overflow-x-auto rounded-2xl border border-brand-accent bg-white p-2 shadow-sm" role="tablist">
                <div class="flex min-w-max gap-2">
                    <button type="button"
                        class="oneshot-tab-btn rounded-xl border-b-2 border-emerald-600 bg-emerald-50 px-4 py-3 text-sm font-black text-emerald-700"
                        data-tab="master" role="tab" aria-selected="true">
                        <i class="fas fa-bolt mr-2" aria-hidden="true"></i>Master Quick Add
                    </button>
                    <button type="button"
                        class="oneshot-tab-btn rounded-xl border-b-2 border-transparent px-4 py-3 text-sm font-bold text-brand-slate hover:bg-brand-offwhite"
                        data-tab="item" role="tab" aria-selected="false">
                        <i class="fas fa-box mr-2" aria-hidden="true"></i>Tambah Item Baru
                    </button>
                    <button type="button"
                        class="oneshot-tab-btn rounded-xl border-b-2 border-transparent px-4 py-3 text-sm font-bold text-brand-slate hover:bg-brand-offwhite"
                        data-tab="masuk" role="tab" aria-selected="false">
                        <i class="fas fa-arrow-down mr-2" aria-hidden="true"></i>Transaksi Masuk
                    </button>
                    <button type="button"
                        class="oneshot-tab-btn rounded-xl border-b-2 border-transparent px-4 py-3 text-sm font-bold text-brand-slate hover:bg-brand-offwhite"
                        data-tab="keluar" role="tab" aria-selected="false">
            ...
            <section id="tab-content-master" class="oneshot-tab-content" role="tabpanel">
                <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                    ...
                    @foreach ($masters as $master)
                        <form action="{{ route($master['route']) }}" method="POST"
                            class="ajax-micro-form rounded-3xl border border-brand-accent bg-white p-5 shadow-sm">
                            @csrf
                            <div class="mb-5 flex items-center justify-between gap-3">
                                <h2 class="text-lg font-black text-brand-emerald">{{ $master['title'] }}</h2>
                                <span
                                    class="rounded-lg bg-brand-offwhite px-2 py-1 text-[10px] font-black uppercase tracking-widest text-brand-slate">Quick
                                    Input</span>
                            </div>
                            <div class="space-y-4">
                                @foreach ($master['fields'] as $field)
                                    <div>
                                        <label for="{{ $master['title'] }}-{{ $field['name'] }}"
                                            class="field-label">{{ $field['label'] }}</label>
                                        @if ($field['type'] === 'textarea')
                                            <textarea id="{{ $master['title'] }}-{{ $field['name'] }}" name="{{ $field['name'] }}" rows="2"
                                                class="field-input"></textarea>
                                        @else
                                            <input id="{{ $master['title'] }}-{{ $field['name'] }}"
                            ...
                            <button type="submit"
                                class="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-brand-emerald px-4 py-3 text-sm font-black text-white transition hover:bg-brand-slate">
            ...
            <!-- TAB 2: TAMBAH ITEM BARU (HYBRID INPUT + VOICE ASSISTANT) -->
            <section id="tab-content-item" class="oneshot-tab-content hidden" role="tabpanel">

                <!-- BARIS VOICE ASSISTANT BUTTON -->
                    ...
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            ...
                            <p class="mt-1 text-xs text-brand-slate leading-relaxed">
                                ...
                                <span class="font-semibold text-emerald-800">Contoh:</span> <em>"Tambah item Cattleya
                                    Mantini, kategori Bunga Tangkai, satuan Ikat, stok 30, harga 5 ribu, deskripsi ini bunga
                                    impor"</em>
                            </p>
                        </div>
                        <button type="button" id="btn-voice-input"
                            class="inline-flex shrink-0 items-center justify-center gap-3 rounded-2xl bg-brand-emerald px-6 py-4 text-sm font-black text-white shadow-md transition hover:bg-emerald-800 active:scale-95">
                ...
                <!-- FORM UTAMA -->
                <form id="form-tambah-item" action="{{ route('form.item.store') }}" method="POST"
                    ...
                    <div class="grid gap-5 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <div class="relative hybrid-select" id="hybrid-kategori">
                                <input type="text" name="kategori_input" id="kategori_input"
                                    value="{{ old('kategori_input') }}" required class="field-input pr-10 hybrid-input"
                                    placeholder="-- Pilih / Ketik Kategori --" autocomplete="off">
                                <button type="button"
                        ...
                        <div>
                            ...
                            <div class="relative hybrid-select" id="hybrid-satuan">
                                <input type="text" name="satuan_input" id="satuan_input"
                                    value="{{ old('satuan_input') }}" required class="field-input pr-10 hybrid-input"
                                    placeholder="-- Pilih / Ketik Satuan --" autocomplete="off">
                                <button type="button"
                                    class="absolute inset-y-0 right-0 flex items-center pr-3 text-brand-slate hybrid-toggle">
                                    <i class="fas fa-chevron-down text-xs pointer-events-none"></i>
                                </button>
                        ...
                        <div>
                            ...
                            <input name="harga_dasar" id="input-harga-dasar" type="number" min="0"
                                step="0.01" required class="field-input" placeholder="0.00">
                        </div>
                        <div>
                ...
                <form action="{{ route('barang-masuk.store') }}" method="POST"
                    ...
                    <div class="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
                        ...
                        <div>
                            ...
                            <input name="harga_satuan" type="number" min="0" step="0.01" required
                                class="field-input">
                        </div>
                        <div>
                            <label class="field-label">Tanggal Masuk</label>
                            <input name="tanggal_masuk" type="date" required value="{{ date('Y-m-d') }}"
                                class="field-input">
                        </div>
                        <div>
            ...
            <section id="tab-content-keluar" class="oneshot-tab-content hidden" role="tabpanel">
                <form action="{{ route('barang-keluar.store') }}" method="POST"
                    ...
                    <div class="mb-5 flex min-h-8 items-center">
                        <span id="live-stock-badge" class="text-xs font-black text-brand-slate"
                            aria-live="polite"></span>
                    </div>
                    <div class="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
                        <div>
                            <label class="field-label">Pilih Barang</label>
                            <select id="select-item-outbound" name="kode_barang" required
                                class="field-input select-item-kode">
                                <option value="">-- Pilih Barang --</option>
                                @foreach ($items ?? [] as $item)
                                    <option value="{{ $item->kode_barang }}">{{ $item->kode_barang }} —
                                        {{ $item->nama_barang }}</option>
                        ...
                        <div>
                            ...
                            <input id="input-harga-jual" name="harga_jual" type="number" min="0" step="0.01"
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // ====== Refresh Dropdowns & Hybrid Options ======
        async function refreshDropdowns() {
            try {
                const response = await fetch('{{ route('form.options') }}', {
                ...
                document.querySelectorAll('.select-item-kode').forEach((select) => {
                    ...
                    (data.items || []).forEach((item) => {
                        const option = new Option(
                            `${item.kode_barang} — ${item.nama_barang}`, item.kode_barang
                        );
                ...
                // Update Hybrid Options (Kategori & Satuan)
                const updateHybridOptions = (wrapperId, items) => {
                    ...
                    (items || []).forEach((item) => {
                        ...
                        li.className =
                            'hybrid-option cursor-pointer px-4 py-2 text-sm hover:bg-emerald-50 hover:text-emerald-700 font-medium';
        ...
        // ====== Live Stock Lookup ======
        const outboundSelect = document.querySelector('#select-item-outbound');
        const stockBadge = document.querySelector('#live-stock-badge');
        const hargaJualInput = document.querySelector('#input-harga-jual');

        if (outboundSelect) {
            outboundSelect.addEventListener('change', async (event) => {
        ...
        // ====== Voice Input (Speech-to-Text + AI Fill) ======
        ...
        // Jika tombol voice tidak ada, lewati seluruh bagian ini
        if (btnVoice) {
            const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;

            if (!SpeechRecognition) {
                btnVoice.disabled = true;
                if (voiceStatusText) voiceStatusText.textContent = 'Browser Tidak Mendukung Voice';
            } else {
                ...
                const resetButtonUI = () => {
                    isListening = false;
                    btnVoice.classList.replace('bg-rose-600', 'bg-brand-emerald');
                    btnVoice.classList.remove('animate-pulse');
                    if (voiceIcon) voiceIcon.className = 'fas fa-microphone text-lg';
                    if (voiceStatusText) voiceStatusText.textContent = 'Mulai Bicara';
                };
                ...
                recognition.onstart = () => {
                    isListening = true;
                    btnVoice.classList.replace('bg-brand-emerald', 'bg-rose-600');
                    btnVoice.classList.add('animate-pulse');
                    if (voiceIcon) voiceIcon.className = 'fas fa-microphone-slash text-lg';
                    if (voiceStatusText) voiceStatusText.textContent = 'Mendengarkan... (Klik untuk stop)';
                    if (transcriptBox) transcriptBox.classList.remove('hidden');
                    if (transcriptText) transcriptText.textContent = 'Silakan bicara bebas...';
                };
                ...
                recognition.onerror = (event) => {
                    if (voiceStatusText) voiceStatusText.textContent = 'Error: ' + event.error;
                    resetButtonUI();
                };

                recognition.onresult = async (event) => {
                    const spokenText = event.results[0][0].transcript;
                    if (transcriptText) transcriptText.textContent = `"${spokenText}" (Memproses AI...)`;
                    if (voiceStatusText) voiceStatusText.textContent = 'AI Menganalisis...';

                    try {
                        console.log("URL:", '{{ route('form.parse-voice') }}');
                        ...
                        if (result.success && result.data) {
                            ...
                            // Auto-fill form dengan pengecekan keberadaan elemen
                            const setValue = (id, value) => {
                                const el = document.getElementById(id);
                                if (el) el.value = value ?? '';
                            };

                            setValue('input-nama-barang', d.nama_barang);
                            setValue('kategori_input', d.kategori_input);
                            setValue('satuan_input', d.satuan_input);
                            setValue('input-stok-minimum', d.stok_minimum);
                            setValue('input-harga-dasar', d.harga_dasar);
                            setValue('input-deskripsi', d.deskripsi);

                            if (voiceStatusText) voiceStatusText.textContent = 'Selesai! Form Terisi';
                            if (typeof toast === 'function') toast('Data berhasil diisi oleh AI!', 'success');
                        } else {
                            throw new Error(result.message || 'Gagal memproses AI');
                        }
                    } catch (err) {
                        if (voiceStatusText) voiceStatusText.textContent = 'Gagal memproses. Coba Lagi';
