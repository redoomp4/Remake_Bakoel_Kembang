...
@section('content')
            ...
            <section class="space-y-8">
                ...
                <div
                    ...
                    <div>
                        ...
                        <p class="text-brand-slate font-medium text-md mt-1">Urusan uang kebun aman terkendali. Star utama
                            laci kas harian.</p>
                    </div>
                    <div
    ...
    <!-- Card 2: Barang Masuk -->
    <div
        class="bg-white border-2 border-brand-accent/60 p-6 rounded-3xl shadow-sm relative overflow-hidden group hover:shadow-md transition-shadow">
        ...
        <span
            class="text-[10px] font-black uppercase tracking-widest text-brand-emerald bg-emerald-50 border border-emerald-100 px-3 py-1 rounded-md">
            💰 UANG MASUK (OMZET)
        </span>
    ...
    <div
        ...
        <span
            class="text-[10px] font-black uppercase tracking-widest text-brand-slate bg-gray-50 border border-gray-100 px-3 py-1 rounded-md">
    ...
    <div class="bg-white rounded-3xl border border-brand-accent p-6 md:p-8 shadow-sm space-y-6">
        <div>
            ...
            <p class="text-brand-slate text-xs font-medium">Bandingan kas masuk (penerimaan) vs kas keluar (pengeluaran)
                secara visual.</p>
        </div>
        ...
        <div class="space-y-4">
            ...
            <div class="space-y-1.5">
                <div class="flex justify-between items-center text-sm font-bold">
                    <span class="text-brand-emerald font-black"><i class="fas fa-arrow-trend-up"></i> UANG MASUK
                        (OMZET)</span>
                    <span class="font-mono">{{ $masukHariIni }} item</span>
                </div>
                <div class="w-full bg-gray-100 h-5 rounded-full overflow-hidden">
                    <div class="bg-brand-emerald h-full transition-all duration-500 rounded-full"
            ...
            <div class="space-y-1.5">
                <div class="flex justify-between items-center text-sm font-bold">
                    <span class="text-brand-slate font-black"><i class="fas fa-arrow-trend-down"></i> UANG KELUAR
                        (MODAL)</span>
                    <span class="font-mono">{{ $keluarHariIni }} item</span>
                </div>
                <div class="w-full bg-gray-100 h-5 rounded-full overflow-hidden">
                    <div class="bg-brand-slate h-full transition-all duration-500 rounded-full"
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-100 font-medium text-md">
                            ...
                            @empty
                                <tr>
                                    <td colspan="3" class="py-8 text-center text-sm font-semibold text-gray-400">Belum
                                        ada data transaksi masuk.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <div class="space-y-6">
                ...
                <div class="space-y-2 text-md">
                    @forelse($stokMinimum as $item)
                        <div
                            ...
                            <div>
                                <span class="block text-gray-900 font-extrabold">{{ $item->nama_barang }}</span>
                                <span class="text-[10px] text-brand-slate uppercase tracking-wider block mt-0.5">Stok min:
                                    {{ $item->stok_minimum }}</span>
                            </div>
                            <span class="bg-rose-600 text-white font-black px-3 py-1 rounded-lg text-xs">Sisa:
                                {{ $item->stok_akhir }}</span>
                        </div>
                    @empty
                        <p class="text-gray-400 font-semibold text-center text-xs py-4">Aman! Seluruh tanaman mencukupi
                            ambang batas.</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl border border-brand-accent p-6 shadow-sm space-y-4">
                ...
                <div class="space-y-2 text-md">
                    ...
                    @empty
                        <p class="text-gray-400 font-semibold text-center text-xs py-4">Tidak ada barang kadaluarsa dalam
                            30 hari ke depan.</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl border border-brand-accent p-6 shadow-sm space-y-4">
                ...
                <div
                    ...
                    <span
                        class="text-brand-emerald font-black text-lg">{{ count($stokTerendah) > 0 ? count($stokTerendah) . '+ item' : '0 item' }}</span>
                </div>
            </div>
    ...
    <div
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                ...
                <p class="text-xs font-semibold text-brand-sage mt-1">Rincian lengkap omzet, modal, laba bersih, & nilai
                    aset inventaris kebun Anda.</p>
            </div>
        </div>
