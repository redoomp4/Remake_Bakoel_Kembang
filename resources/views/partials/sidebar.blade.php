@php
    $userRole = strtolower(auth()->user()->role ?? 'viewer');
...
<aside class="w-full lg:w-80 bg-white border-r border-brand-accent p-6 flex flex-col gap-3 shrink-0">
  <div class="mb-4 px-2 py-3 border-b border-brand-accent/50">
    <p class="text-xs font-black text-brand-slate uppercase tracking-widest">
      {{ $isViewer ? 'Akses Viewer Logistik' : 'Akses Kebun (' . strtoupper($userRole) . ')' }}
    </p>
  ...
  <nav class="flex flex-col gap-2">
    ...
    <!-- Laporan Transaksi (Single Complete Report Page) -->
