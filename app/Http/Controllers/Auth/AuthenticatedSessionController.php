    ...
    public function store(LoginRequest $request): RedirectResponse
    {
        ...
        if ($user->role === 'gudang') {
            ...
            foreach ($lowStockItems as $item) {
                $stok_akhir =
                    (int) $item->total_masuk -
                    (int) $item->total_keluar;

                if (
                    (int) $item->total_masuk > 0 &&
                    $stok_akhir <= (int) $item->stok_minimum
                ) {
                    $latestData = DB::table('barang_masuks')
                        ->join(
                            'lokasis',
                            'barang_masuks.id_lokasi',
                            '=',
                            'lokasis.id'
                        )
                        ->join(
                            'kondisis',
                            'barang_masuks.id_kondisi',
                            '=',
                            'kondisis.id'
                        )
                        ->where(
            ...
            foreach ($slowMovingItems as $item) {
                $lastIn = $item->last_in
                    ? Carbon::parse($item->last_in)
                    : null;

                $lastOut = $item->last_out
                    ? Carbon::parse($item->last_out)
                    : null;
        ...
        return redirect()->intended(
            RouteServiceProvider::redirectByRole()
        );
    }
